...
/** Socket handler type */
typedef char (*sock_handler_func)(buffer_t *buffer);
...
//extern char stack_service_init( stack_event_t stack_event, sock_handler_func stack_event_handler, uint8_t gateway_discover , gateway_cache_t *gw_table );
...
extern char socket_close (socket_t * si);

extern char socket_bind (socket_t * si,
                        sockaddr_t * sa) ;


extern char socket_connect(socket_t *si, sockaddr_t *dst);

extern socket_t *socket_accept(socket_t *si, sockaddr_t *dst,
                 sock_handler_func sock_handler );

extern char socket_sendto (socket_t *si, sockaddr_t *dst, buffer_t *buf);

extern buffer_t *socket_read(socket_t * si, uint16_t time);
extern char socket_write (socket_t *si, buffer_t *buf);

extern socket_t *socket_find (uint16_t port, sockaddr_t *sa);

extern buffer_t *socket_buffer_get(socket_t * si);
extern char socket_buffer_free(buffer_t *buffer);

extern char socket_up(buffer_t *b);
#endif /* _NS_SOCKET_H */
