...
typedef uint32_t    stack_event_t;
...
extern char stack_buffer_add(buffer_t *b);
extern buffer_t *stack_buffer_pull(void);

extern buffer_t* stack_buffer_get ( MQX_TICK_STRUCT blocktime );
extern char stack_buffer_free ( buffer_t *b );
extern char stack_buffer_push( buffer_t * b);
extern char stack_buffer_headroom( buffer_t * b, uint16_t size);
...
extern char stack_check_broadcast(uint8_t *address, addrtype_t type);

extern char udp_echo(sockaddr_t *dst, discover_res_t  *result_ptr);
extern char ping(sockaddr_t *dst , discover_res_t  *result_ptr);
extern void push_to_app(buffer_t *buf);
extern char parse_echo_response(buffer_t *buf);
extern void stop_ping(void);
extern char gw_discover(void);
extern char gw_advertisment(void);

#if 0
extern void nwk_manager_launch(void);
extern void scan_network(void);
extern char remove_gw_info(sockaddr_t *adr);
extern char update_gw_info_ttl(void);
extern char select_best_gw(sockaddr_t *adr);
extern char gw_table_update(buffer_t *buf);
#endif
...
#endif
extern char stack_compare_address(sockaddr_t *a1, sockaddr_t *a2);
