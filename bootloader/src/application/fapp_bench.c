...
static void fapp_bench_tcp_rx (fnet_shell_desc_t desc)
{
    ...
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_S, "Local IP addr.", inet_ntoa(*(struct in_addr *)( &local_ip), ip_str) );
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_D, "Local port", FNET_NTOHS(FAPP_BENCH_PORT));
...
static void fapp_bench_tcp_tx (struct fapp_bench_tx_params *params)
{
    ...
    fnet_shell_println(desc, FTE_BL_DELIMITER_STR);
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_S, "Remote IP addr.", inet_ntoa(*(struct in_addr *)( &params->ip_address), ip_str));
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_D, "Remote port", fnet_ntohs(params->port));
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_D, "Message size", params->packet_size);
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_D, "Number of messages", params->packet_number);
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_D, "Number of iterations", params->iteration_number);
    fnet_shell_println(desc, FTE_BL_TOCANCEL_STR);
    fnet_shell_println(desc, FTE_BL_DELIMITER_STR);
...
void fapp_benchtx_cmd( fnet_shell_desc_t desc, int argc, char ** argv )
{
    ...
    if( inet_aton(argv[1], (struct in_addr *) &bench_params.ip_address) == FNET_OK) /* TFTP server IP*/
    {
        ...
        if(argc > 3)
        {
            ...
            {
                fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[3]); /* Print error mesage. */
                return;
            }
            ...
            if(argc > 4)
            {
                ...
                {
                    fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[4]); /* Print error mesage. */
                    return;
                }
                ...
                if(argc > 5)
                {
                    ...
                    {
                        fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[5]); /* Print error mesage. */
                        return;
                    }
                }
            }
        ...
        {
            fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[2]);
        }
                
    }
    else
    {
        fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[1]);   /* Wrong Benchmark Server IP address. */
        return;
    }
}
