...
static int fapp_http_ssi_echo_handle(char * query, long *cookie)
{
    ...
    if(ssi_buffer_ptr == 0)
    {
        ...
        {
            fnet_ip_addr_t ip_adr = fnet_netif_get_address(fapp_default_netif);
            inet_ntoa(*(struct in_addr *)( &ip_adr), ip_str);
            fnet_snprintf(fapp_http_ssi_buffer, sizeof(fapp_http_ssi_buffer), "%s", ip_str);
        }
        else if (!fnet_strcmp( query, "SUBNET_MASK"))
        {
            fnet_ip_addr_t ip_adr = fnet_netif_get_netmask(fapp_default_netif);
            inet_ntoa(*(struct in_addr *)( &ip_adr), ip_str);
            fnet_snprintf(fapp_http_ssi_buffer, sizeof(fapp_http_ssi_buffer), "%s", ip_str);
        }
        else if (!fnet_strcmp( query, "GATEWAY"))
        {
            fnet_ip_addr_t ip_adr = fnet_netif_get_gateway(fapp_default_netif);
            inet_ntoa(*(struct in_addr *)( &ip_adr), ip_str);
            fnet_snprintf(fapp_http_ssi_buffer, sizeof(fapp_http_ssi_buffer), "%s", ip_str);
        }
...
static int fapp_http_cgi_stdata_handle(char * query, long *cookie)
{
	...
	/* Get statistics. */
    fnet_memset( &statistics, 0, sizeof(struct fnet_netif_statistics) );
...
static unsigned int fapp_http_cgi_rand(void)
{
  ...
  return ((fapp_http_cgi_rand_next>>4) & 0xFF) + FAPP_HTTP_CGI_GRAPH_MIN;
}
...
{
    int q1= fapp_http_cgi_rand();
    int q2= fapp_http_cgi_rand();
    int q3= fapp_http_cgi_rand();
    int q4= fapp_http_cgi_rand();
    int q5= fapp_http_cgi_rand();
...
*************************************************************************/
int fapp_http_cmd( fnet_shell_desc_t desc, int argc, char ** argv )
{
    ...
    {
        fnet_memset(&params, 0, sizeof(struct fnet_http_params));
        ...
        if(http_desc != FNET_ERR)
        {
            ...
            fnet_shell_println(desc, "  Use: http://%s", ip_str);
            fnet_shell_println(desc, FTE_BL_DELIMITER_STR);
        ...
        {
            fnet_shell_println(desc, FTE_BL_INIT_ERR, "HTTP");
            return  -1;
        }
...
{
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_S, "HTTP server",
                fnet_http_state(fapp_http_desc) != FNET_HTTP_STATE_DISABLED ? FTE_BL_SHELL_INFO_ENABLED: FTE_BL_SHELL_INFO_DISABLED);
}
