...
#if FTE_BL_CFG_TFTP_CMD || FTE_BL_CFG_TFTPUP_CMD
...
#endif /* FTE_BL_CFG_TFTPS_CMD */


#if FTE_BL_CFG_TFTP_CMD || FTE_BL_CFG_TFTPUP_CMD || FTE_BL_CFG_TFTPS_CMD || FTE_BL_CFG_SETGET_CMD_TYPE  
...
* DESCRIPTION: 
************************************************************************/
static void fapp_tftp_tx_image_begin_end(unsigned char **data_begin_p, unsigned char **data_end_p)
{
...
static int fapp_tftp_rx_handler_raw (fapp_tftp_handler_control_t *tftp_handler, fnet_shell_desc_t desc, unsigned char* data, unsigned long n)
{
    ...
    if(raw->dest == 0) /* Only one time. */
        raw->dest = xDynamicParams.tftp.file_raw_address;    
        ...
        switch(srec->state)
        {
            ...
            case FAPP_TFTP_RX_HANDLER_SREC_STATE_GETDATA:
                ...
                if(srec->record_hex_index > ((srec->record.count<<1)))
                {
                    ...
                    {
                        /* Set entry point. */
                        xDynamicParams.boot.go_address = (unsigned long)addr;
...
static inline void fapp_tftp_tx_srec_gen (struct fapp_tftp_tx_handler_srec *tx_srec)
{
    ...
    switch(tx_srec->state)
    {
        ...
        case FAPP_TFTP_TX_HANDLER_SREC_STATE_EOB:
            ...
            checksum = 5;
            /* Starting address.*/
            fnet_sprintf(tx_srec->srec_line.address, "%08X", xDynamicParams.boot.go_address);
            /* Calculate checksum on address field. */
            checksum += (unsigned char)(xDynamicParams.boot.go_address + (xDynamicParams.boot.go_address>>8) 
                       + (xDynamicParams.boot.go_address>>16)+ (xDynamicParams.boot.go_address>>24));
            /* Checksum.*/
...
#endif /* FTE_BL_CFG_TFTP_CMD || FTE_BL_CFG_TFTPUP_CMD || FTE_BL_CFG_TFTPS_CMD */
...
************************************************************************/
int fapp_tftp_cmd( fnet_shell_desc_t desc, int argc, char ** argv )
{
    ...
    tftp_params.file_name = xDynamicParams.tftp.file_name;
    
    tftp_params.server_ip = xDynamicParams.tftp.server;  /* TFTP Server IP address. */
    
    tftp_params.handler_param = (void *)desc;
    tftp_params.timeout = 0; /* Default timeout. */
    
    /* Reset fapp_tftp_handler_control. */
    fnet_memset(&fapp_tftp_handler_control, 0, sizeof(fapp_tftp_handler_control));
    
    fapp_tftp_handler_control.current_type = fapp_tftp_image_type_by_index(xDynamicParams.tftp.file_type); /* Get type. */
    ...
    if(argc >= 2) 
    {
        ...
        {
            if( inet_aton(argv[2], (struct in_addr *) &tftp_params.server_ip) == FNET_OK) /* TFTP server IP*/
            {
                if(argc >= 4 && ((fapp_tftp_handler_control.current_type = fapp_tftp_image_type_by_name (argv[3]))==0)) /* image type */
                {
                    fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[3]); /* Wrong image type. */
                    fnet_shell_script_release(desc);   /* Clear script. */    
                    return -1; 
                }
            }
            ...
            {
                fnet_shell_println(desc, FTE_BL_PARAM_ERR, argv[2]);   /* Wrong TFTP Server IP address. */
                fnet_shell_script_release(desc);                    /* Clear script. */    
                return -1;
            }
        }
        ...
        (fnet_tftp_cln_init( &tftp_params ) != FNET_ERR))
    {
        ...
        fnet_shell_printf(desc, (tftp_params.request_type == FNET_TFTP_REQUEST_READ)?FAPP_TFTP_RX_HEADER_STR:FAPP_TFTP_TX_HEADER_STR, 
                          ...
                          inet_ntoa(*(struct in_addr *)(&tftp_params.server_ip), ip_str));  
        fnet_shell_block(desc, fapp_tftp_on_ctrlc);
    }
    else
    {
        fnet_shell_println(desc, FTE_BL_INIT_ERR, "TFTP");
...
#endif /*FTE_BL_CFG_TFTP_CMD || FTE_BL_CFG_TFTPUP_CMD */
                                                ...
                                                void *shl_desc)
{
    ...
    if(fnet_strcmp(xDynamicParams.tftp.file_name, filename) == 0)
    {
        /* Reset fapp_tftps_handler_control. */
        fnet_memset(&fapp_tftps_handler_control, 0, sizeof(fapp_tftps_handler_control));
    
        /* Get  handler type. */
        fapp_tftps_handler_control.current_type = fapp_tftp_image_type_by_index(xDynamicParams.tftp.file_type); 
           ...
           ((request_type == FNET_TFTP_REQUEST_READ) && fapp_tftps_handler_control.current_type->tx_handler)))
        {
            fnet_shell_println(desc, (request_type == FNET_TFTP_REQUEST_READ)?FAPP_TFTP_TX_HEADER_STR:FAPP_TFTP_RX_HEADER_STR, 
                               ...
                               inet_ntoa(*(struct in_addr *)(&ip_addr), ip_str));
                
            /* Do erase all, on WRITE request.*/
            if(request_type == FNET_TFTP_REQUEST_WRITE)
            {
                fnet_shell_script(desc, FTE_BL_CFG_TFTPS_ON_WRITE_REQUEST_SCRIPT);
            }
    ...
    if(request == FNET_TFTP_REQUEST_WRITE)
    {
            ...
            else
            {
                ...
                if(data_size < FNET_TFTP_DATA_SIZE_MAX)
                {
                    ...
                    fnet_shell_script(desc, FTE_BL_CFG_TFTPS_AFTER_WRITE_REQUEST_SCRIPT);
                }
            }
    }
    /* REQUEST_READ */
...
/************************************************************************
* NAME: fapp_tftps_complete_handler
*
* DESCRIPTION: TFTP server complete handler.
*************************************************************************/
static void fapp_tftps_complete_handler(fnet_tftp_request_t request,
                                                int status,
                                                void *shl_desc)
{                                        
    fnet_shell_desc_t desc = (fnet_shell_desc_t)shl_desc;
    
    fnet_shell_println(desc, "  TFTP server completed.");
}
...
*************************************************************************/
int fapp_tftps_cmd( fnet_shell_desc_t desc, int argc, char ** argv )
{
    char ip_str[16];
    ...
    if(argc == 1) /* By default is "init".*/
    {
        ...
        fnet_memset(&params, 0, sizeof(params));
        ...
        params.complete_handler = fapp_tftps_complete_handler;
        ...
        {
            struct in_addr addr;
            addr.s_addr = fnet_netif_get_address(fapp_default_netif);
            fnet_shell_println(desc, FTE_BL_DELIMITER_STR);
            fnet_shell_println(desc, "  TFTP server (%s) started.", inet_ntoa(addr, ip_str));
            fnet_shell_println(desc, FTE_BL_DELIMITER_STR);
            fapp_tftp_srv_desc = tftp_srv_desc;
        }
        else
        {
            fnet_shell_println(desc, FTE_BL_INIT_ERR, "TFTP Server");
            return  -1;
        }
...
{
    fnet_shell_println(desc, FTE_BL_SHELL_INFO_FORMAT_S, "TFTP server",
                fnet_tftp_srv_state(fapp_tftp_srv_desc) != FNET_TFTP_SRV_STATE_DISABLED ? FTE_BL_SHELL_INFO_ENABLED: FTE_BL_SHELL_INFO_DISABLED);
}
