...
*     Function Prototypes
*************************************************************************/
void fapp_set_cmd_id(fnet_shell_desc_t desc, char *value );
void fapp_get_cmd_id(fnet_shell_desc_t desc);
void fapp_set_cmd_model(fnet_shell_desc_t desc, char *value );
void fapp_get_cmd_model(fnet_shell_desc_t desc);
...
/************************************************************************
* NAME: fapp_set_cmd_id
*
* DESCRIPTION: Set device ID
************************************************************************/
#if FTE_BL_CFG_SETGET_CMD_ID
static void fapp_set_cmd_id(fnet_shell_desc_t desc, char *value )
{
    fnet_strncpy( xStaticParams.device.id, value, FTE_BL_PARAMS_DEVICE_ID_SIZE);
}
#endif
/************************************************************************
* NAME: fapp_get_cmd_id
*
* DESCRIPTION: Gets device ID.
************************************************************************/
#if FTE_BL_CFG_SETGET_CMD_ID
static void fapp_get_cmd_id(fnet_shell_desc_t desc)
{
    fnet_shell_println(desc, FAPP_GET_SOPT_FORMAT, xStaticParams.device.id);
}
#endif

/************************************************************************
* NAME: fapp_set_cmd_model
*
* DESCRIPTION: Set device Model
************************************************************************/
#if FTE_BL_CFG_SETGET_CMD_MODEL
static void fapp_set_cmd_model(fnet_shell_desc_t desc, char *value )
{
    fnet_strncpy( xStaticParams.device.model, value, FTE_BL_PARAMS_MODEL_SIZE);
}
#endif
/************************************************************************
* NAME: fapp_get_cmd_model
*
* DESCRIPTION: Gets device Model.
************************************************************************/
#if FTE_BL_CFG_SETGET_CMD_MODEL
static void fapp_get_cmd_model(fnet_shell_desc_t desc)
{
    fnet_shell_println(desc, FAPP_GET_SOPT_FORMAT, xStaticParams.device.model);
}
#endif
...
static void fapp_set_ip(fnet_shell_desc_t desc, char *value, void (*set_ip)( fnet_netif_desc_t netif_desc, fnet_ip_addr_t ipaddr ) )
{
    ...
    {
        fnet_shell_println(desc, FTE_BL_PARAM_ERR, value);
    };
}
...
static void fapp_set_cmd_mac(fnet_shell_desc_t desc, char *value)
{
    ...
    {
            fnet_memcpy(xStaticParams.device.mac, macaddr, sizeof(fnet_mac_addr_t));
            fnet_shell_println(desc, FTE_BL_PARAM_ERR, value);
    }
}
#endif
...
{
    (void)desc;
    fnet_strncpy( xDynamicParams.boot.script, value, FTE_BL_PARAMS_BOOT_SCRIPT_SIZE);
}
#endif
/************************************************************************
...
{
    fnet_shell_println(desc, FAPP_GET_SOPT_FORMAT, xDynamicParams.boot.script);
}
#endif
/************************************************************************
...
{
    const struct boot_mode *mode = FTE_BL_bootModeByName(value);

    if(mode == 0)
        fnet_shell_println(desc, FTE_BL_PARAM_ERR, value);
    else
        xDynamicParams.boot.mode = mode->index;    
...
{
    const struct boot_mode *mode = FTE_BL_bootModeByIndex(xDynamicParams.boot.mode);
...
static void fapp_set_cmd_bootdelay(fnet_shell_desc_t desc, char *value )
{
    ...
    if ((delay == 0) && (p == value))
        fnet_shell_println(desc, FTE_BL_PARAM_ERR, value); /* Print error mesage. */
    else
        xDynamicParams.boot.delay = delay;     
}
#endif
/************************************************************************
...
{
    fnet_shell_println(desc, FAPP_GET_DOPT_FORMAT, xDynamicParams.boot.delay);
}
#endif
...
static void fapp_set_cmd_tftp(fnet_shell_desc_t desc, char *value )
{
    ...
    if(inet_aton(value, (struct in_addr *) &addr) == FNET_OK)
    {
        xDynamicParams.tftp.server = addr;
    }
    else
    {
        fnet_shell_println(desc, FTE_BL_PARAM_ERR, value); /* Print error. */
    }
...
static void fapp_get_cmd_tftp(fnet_shell_desc_t desc)
{
    char ip_str[16];
    struct in_addr addr;
    addr.s_addr = xDynamicParams.tftp.server;
    inet_ntoa(addr, ip_str);
    fnet_shell_println(desc, FAPP_GET_SOPT_FORMAT, ip_str);
}
#endif
...
{
    (void)desc;
    fnet_strncpy( xDynamicParams.tftp.file_name, value, FTE_BL_PARAMS_TFTP_FILE_NAME_SIZE);
}
#endif
/************************************************************************
...
{
    fnet_shell_println(desc, FAPP_GET_SOPT_FORMAT,xDynamicParams.tftp.file_name);
}
#endif
/************************************************************************
...
static void fapp_set_cmd_image_type(fnet_shell_desc_t desc, char *value )
{
    ...
    if(type == 0)
        ...
        xDynamicParams.tftp.file_type = type->index;
...
{
    struct image_type *type = fapp_tftp_image_type_by_index(xDynamicParams.tftp.file_type);
...
static void fapp_set_cmd_go(fnet_shell_desc_t desc, char *value )
{
    ...
    if ((address == 0) && (p == value))
        ...
        xDynamicParams.boot.go_address = address;     
}
#endif
/************************************************************************
...
{
    fnet_shell_println(desc, FAPP_GET_XOPT_FORMAT, xDynamicParams.boot.go_address);
}
#endif
/************************************************************************
...
static void fapp_set_cmd_raw(fnet_shell_desc_t desc, char *value )
{
    ...
    if ((address == 0) && (p == value))
        ...
        xDynamicParams.tftp.file_raw_address = address;     
}
#endif
/************************************************************************
...
{
    fnet_shell_println(desc, FAPP_GET_XOPT_FORMAT, xDynamicParams.tftp.file_raw_address);
}
#endif
/************************************************************************
* NAME: fapp_set_cmd
...
************************************************************************/
int fapp_set_cmd( fnet_shell_desc_t desc, int argc, char ** argv )
{
    int index;
    ...
    {
        fnet_shell_println(desc, FTE_BL_PARAM_ERR, "");
        return  -1;
    }
    else /* Set parameter */
    {
...
************************************************************************/
int fapp_get_cmd( fnet_shell_desc_t desc, int argc, char ** argv )
{
    int index;
