...
int fnet_shell_make_argv( char *cmdline, char *argv [] );
...
static void fnet_shell_state_machine( void *shell_if_p )
{
    ...
    int ch;
    int argc;
    char *_argv[FNET_CFG_SHELL_ARGS_MAX + 1]; /* One extra for 0 terminator.*/
    char **argv; /* One extra for 0 terminator.*/
    ...
    switch(shell_if->state)
    {
        ...
        case FNET_SHELL_STATE_GET_USER_INPUT:
            if( (ch = fnet_serial_getchar(shell_if->stream)) != FNET_ERR) 
            {
                ...
                if((ch != FNET_SHELL_LF) && (ch != FNET_SHELL_CR) && (shell_if->pos < shell_if->cmd_line_size))
                {
                    switch(ch)
                    {
        ...
        case FNET_SHELL_STATE_EXEC_CMD:
            ...
            argc = fnet_shell_make_argv(shell_if->cmd_line, _argv);
            if ((_argv[0][0] == '!') && (fnet_shell_cmd_ret((fnet_shell_desc_t)shell_if) != 0))
            {
                argv = &_argv[1];
                argc--;
            }
            else if ((strcmp(_argv[0], "clr_ret") == 0) || 
                     ((_argv[0][0] != '!') && (fnet_shell_cmd_ret((fnet_shell_desc_t)shell_if) == 0)))
            {
                argv = &_argv[0];
            }
            else
                                       ...
                                       && ((argc - 1) <= cur_command->max_args))
                        {
                            ...
                            else /* Command. */
                            {
                                if(cur_command->cmd_ptr)
                                            shell_if->cmd_ret = ((int(*)(fnet_shell_desc_t desc, int, char **))(cur_command->cmd_ptr))((fnet_shell_desc_t)shell_if, argc, argv);
...
fnet_shell_desc_t fnet_shell_init( struct fnet_shell_params * params)
{
    ...
    shell_if->service_descriptor = fnet_poll_register(fnet_shell_state_machine, (void *) shell_if);
...
void fnet_shell_release(fnet_shell_desc_t desc)
{
    ...
    {
        fnet_poll_unregister(shell_if->service_descriptor); /* Delete service.*/
...
void fnet_shell_script_release( fnet_shell_desc_t desc)
{
    ...
    if(shell_if)
    {
       fnet_memset(shell_if->cmd_line, '\0', shell_if->cmd_line_size );
    }
}
...
int fnet_shell_ctrlc (fnet_shell_desc_t desc)
{
    ...
    if(fnet_serial_getchar(shell_if->stream) == FNET_SHELL_CTRLC)
        res = 1;
    else
        res = 0;
...
void fnet_shell_set_cmd_ret( fnet_shell_desc_t desc, int ret)
{
    struct fnet_shell_if *shell_if = (struct fnet_shell_if *) desc;
    
    if(shell_if)
    {
        shell_if->cmd_ret = ret;
    }
}


void fnet_shell_clr_ret( fnet_shell_desc_t desc)
{
    struct fnet_shell_if *shell_if = (struct fnet_shell_if *) desc;
...
int fnet_shell_cmd_ret( fnet_shell_desc_t desc)
{
    struct fnet_shell_if *shell_if = (struct fnet_shell_if *) desc;
    
    if(shell_if)
    {
        return  shell_if->cmd_ret;
    }
