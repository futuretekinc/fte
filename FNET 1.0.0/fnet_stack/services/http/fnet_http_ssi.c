...
*     Definitions
************************************************************************/
unsigned long fnet_http_ssi_send (struct fnet_http_if * http);

#if FNET_CFG_HTTP_VERSION_MAJOR /* HTTP/1.x*/
int fnet_http_ssi_handle (struct fnet_http_if * http, struct fnet_http_uri * uri);
#endif

const static char fnet_http_ssi_head[] = {'<','!','-','-','#'};
const static char fnet_http_ssi_tail[] = {'-','-','>'};
...
unsigned long fnet_http_ssi_send (struct fnet_http_if * http)
{
    ...
    while (result<sizeof(http->buffer) && (next == 0))
    {
        if(http->ssi.state != FNET_HTTP_SSI_INCLUDING) /* Read from file if not in including. */
            if((read_result = fnet_fs_fread(buffer, 1, http->send_file)) == 0)
                break; /*EOF*/
        ...
        switch (http->ssi.state)
        {
            case FNET_HTTP_SSI_WAIT_HEAD:
                if(*buffer == fnet_http_ssi_head[ssi_head_index])
                {
                    ...
                    { /* Head is found */
                        ...
                        { /* Found in the middle */
                            fnet_fs_fseek (http->send_file, -sizeof(fnet_http_ssi_head), FNET_FS_SEEK_CUR);
                            next = 1; /* break */
            ...
            case FNET_HTTP_SSI_WAIT_TAIL:
                if(*buffer == fnet_http_ssi_tail[ssi_tail_index])
                {
                    ...
                    { /* Tail is found */
                        const struct fnet_http_ssi *ssi_ptr;
    ...
    { /* Potential SSI is splitted => parse in the next itteration */
        ...
        fnet_fs_fseek (http->send_file, -ssi_head_index, FNET_FS_SEEK_CUR);
    }
