...
#define FNET_HTTP_BACKLOG_MAX   (2)
...
struct fnet_http_status fnet_http_status_list[] =
{
    ...
    void fnet_http_version_parse(char * in_str, struct fnet_http_version * version);
    int fnet_http_tx_status_line (struct fnet_http_if * http);
    ...
    for(iteration = 0; iteration < FNET_HTTP_ITERATION_NUMBER; iteration++)
    {
        switch(http->state)
        {
            ...
            case FNET_HTTP_STATE_LISTENING:
                ...
                if((http->socket_foreign = accept(http->socket_listen, (struct sockaddr *) &foreign_addr, &len))
                     != SOCKET_INVALID)
                {
#if FNET_CFG_DEBUG_HTTP
                    {
                        ...
                        FNET_DEBUG_HTTP("");
                        FNET_DEBUG_HTTP("HTTP: RX Request From: %s; Port: %d.", ip_str, fnet_ntohs(foreign_addr.sin_port));
                    }
#endif

                    /* Reset response & request parameters.*/
                    fnet_memset(&http->response, 0, sizeof(struct fnet_http_response));
                    fnet_memset(&http->request, 0, sizeof(struct fnet_http_request));
                                    ...
                                    if(http->request.skip_line == 0)
                                    {
                                        ...
                                        /* === Parse header fields. ===*/
                                        {
                                            ...
                                            if (http->request.method->receive && fnet_strncmp(req_buf, FNET_HTTP_HEADER_FIELD_CONTENT_LENGTH, sizeof(FNET_HTTP_HEADER_FIELD_CONTENT_LENGTH)-1) == 0)
                                            {
                                                ...
                                                http->request.content_length = fnet_strtoul(length_str,&p,10);
                                            }
    #endif                                            
                                        }
                                    }
...
fnet_http_desc_t fnet_http_init( struct fnet_http_params *params )
{
    ...
    if((http_if->socket_listen = socket(AF_INET, SOCK_STREAM, 0)) == SOCKET_INVALID)
    {
    ...
    if(params->port)    /* Aply the user-defined port. */
        ...
        local_addr.sin_port = FNET_CFG_HTTP_PORT;    

    local_addr.sin_addr.s_addr = params->ip_address;
    local_addr.sin_family = AF_INET;

    if(bind(http_if->socket_listen, (struct sockaddr *)(&local_addr), sizeof(local_addr)) == SOCKET_ERROR)
    {
    ...
    http_if->service_descriptor = fnet_poll_register(fnet_http_state_machine, (void *) http_if);
...
void fnet_http_release(fnet_http_desc_t desc)
{
    ...
    {
        fnet_fs_fclose(http_if->index_file);
        fnet_fs_fclose(http_if->send_file);
        fnet_fs_closedir(http_if->root_dir);

        closesocket(http_if->socket_foreign);        
           
        closesocket(http_if->socket_listen);
        fnet_poll_unregister(http_if->service_descriptor); /* Delete service.*/
...
int fnet_http_tx_status_line (struct fnet_http_if * http)
{
    ...
    do
    {
        ...
        switch(http->response.status_line_state)
        {
            case 0:
                ...
                /* Print status line.*/
                result_state = fnet_snprintf(http->buffer, FNET_HTTP_BUF_SIZE, "%s%d.%d %d %s%s", FNET_HTTP_VERSION_HEADER, http->response.version.major, http->response.version.minor,
            ...
            case 1:
                ...
                {
                    result_state = fnet_snprintf(&http->buffer[result], (FNET_HTTP_BUF_SIZE - result), "%s ", FNET_HTTP_HEADER_FIELD_AUTHENTICATE);
            ...
            case 2:
                ...
                {
                    result_state = fnet_snprintf(&http->buffer[result], (FNET_HTTP_BUF_SIZE - result), "%s %d%s", FNET_HTTP_HEADER_FIELD_CONTENT_LENGTH, http->response.content_length, "\r\n");
                }
                break;                
            case 3:
                /*Final CRLF.*/
                result_state += fnet_snprintf(&http->buffer[result], (FNET_HTTP_BUF_SIZE - result),"%s","\r\n");
    ...
    while (http->response.status_line_state <= 3);
...
int fnet_http_default_handle (struct fnet_http_if * http, struct fnet_http_uri * uri)
{
    ...
    if (!fnet_strcmp(uri->path, "/")) /* Default index file */
    {
        ...
        http->send_file = http->index_file;
    }    
    else
    {
        http->send_file = fnet_fs_fopen_re(uri->path,"r", http->root_dir);
    }
    ...
    if (http->send_file)
    {
#if FNET_CFG_HTTP_VERSION_MAJOR /* HTTP/1.x*/
        {
            ...
            fnet_fs_finfo (http->send_file, &dirent);
            http->response.content_length = dirent.d_size;
        }
#endif        
        result = FNET_OK;
    }
...
{
    return fnet_fs_fread(http->buffer, sizeof(http->buffer), http->send_file);
}
...
{
    if(http->send_file != http->index_file)
        fnet_fs_fclose(http->send_file); /* Close file */ 
}
