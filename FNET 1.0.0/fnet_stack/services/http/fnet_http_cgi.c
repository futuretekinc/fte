...
int fnet_http_cgi_handle (struct fnet_http_if * http, struct fnet_http_uri * uri);
unsigned long fnet_http_cgi_send (struct fnet_http_if * http);
...
static unsigned long fnet_http_cgi_send (struct fnet_http_if * http)
{
    ...
    if(http->send_ptr)
    {
        cgi_ptr = (const struct fnet_http_cgi *) http->send_ptr;
