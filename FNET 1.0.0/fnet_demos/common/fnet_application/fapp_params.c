...
int fapp_params_to_flash()
{
    ...
    {
        ip_address = fapp_params->fnet_params.address;              /* Preserve the old value.*/
    }
    else
    {
    ...
    if( fnet_memcmp((void *)(fapp_params), FAPP_PARAMS_SIGNATURE, sizeof(FAPP_PARAMS_SIGNATURE)) !=0 )
    {
        ...
        fnet_params.address = ip_address;
        fnet_params.netmask = fnet_netif_get_netmask(fapp_default_netif);
        fnet_params.gateway = fnet_netif_get_gateway(fapp_default_netif);
        fnet_netif_get_hw_addr(fapp_default_netif, fnet_params.mac, sizeof(fnet_mac_addr_t));
...
int fapp_params_from_flash()
{
    ...
    if(fnet_strncmp( fnet_params->signature, FAPP_PARAMS_SIGNATURE, FAPP_PARAMS_SIGNATURE_SIZE )==0)
    {
        ...
        fnet_netif_set_address(fapp_default_netif, fnet_params->fnet_params.address); 
        fnet_netif_set_gateway(fapp_default_netif, fnet_params->fnet_params.gateway);       
        fnet_netif_set_netmask(fapp_default_netif, fnet_params->fnet_params.netmask);
        fnet_netif_set_hw_addr(fapp_default_netif, fnet_params->fnet_params.mac, sizeof(fnet_mac_addr_t));
