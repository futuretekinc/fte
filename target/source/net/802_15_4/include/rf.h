...
extern char rf_init(void);

extern char rf_rx_enable(void);
extern char rf_rx_disable(void);

extern char rf_write(buffer_t *buffer);
extern void rf_send_ack(uint8_t pending);
extern int8_t rf_analyze_rssi(void);
extern char rf_cca_check(uint8_t backoff_count, uint8_t slotted);
extern char rf_write_no_cca(buffer_t *buffer);

extern char rf_mac_get(sockaddr_t *address);
extern int8_t rf_power_set(uint8_t new_power);
extern int8_t rf_channel_set(uint8_t channel);

extern void rf_ISR( void );
extern void rf_error_ISR( void );
extern char mac_set_channel(uint8_t new_channel);
extern uint8_t mac_current_channel(void);
extern char mac_mem_alloc(void);
...
#endif
extern char rf_address_decoder_mode(rf_address_mode_t mode);
