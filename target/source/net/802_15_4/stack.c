...
void NS_STACK_main ( uint_32 ulParams)
{
...
#if 0	
	xLastWakeTime = (portTickType) ulParams;
	...
	for(;;)
	{
		...
		if ((_time_diff_milliseconds(&xCurrentTime, &xLastWakeTime, &bOverflow) > 15000) || bOverflow)
		{
			check_tables_status();
            _time_get_ticks(&xLastWakeTime);
		}
