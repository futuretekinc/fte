...
#include "fte_log.h"
#include "fte_time.h"
#if FTE_SHT7X_SUPPORTED

static  
FTE_RET FTE_SHT7X_init
(
    FTE_OBJECT_PTR  pObj
);

static  
FTE_RET FTE_SHT7X_run
(
    FTE_OBJECT_PTR  pObj
);

static  
FTE_RET FTE_SHT7X_stop
(
    FTE_OBJECT_PTR  pObj
);
...
static  
FTE_RET FTE_SHT7X_connectionReset
(
    FTE_OBJECT_PTR  pObj
);

static  
FTE_RET FTE_SHT7X_startTransmission
(
    FTE_OBJECT_PTR  pObj
);

static 
FTE_RET  FTE_SHT7X_startConvert
(
    FTE_OBJECT_PTR  pObj
);

static  
void    FTE_SHT7X_restartConvert
(
    FTE_TIMER_ID    xTimerID, 
    FTE_VOID_PTR    pData, 
    MQX_TICK_STRUCT_PTR pTick
...
static FTE_RET  FTE_SHT7X_getHumidity
(
    FTE_OBJECT_PTR  pObj, 
    FTE_UINT32_PTR  pHumidity
);

static FTE_RET  FTE_SHT7X_getTemperature
(
    FTE_OBJECT_PTR  pObj, 
...
static 
FTE_RET FTE_SHT7X_getUpdateInterval
(
    FTE_OBJECT_PTR  pObj,
...
static 
FTE_RET  FTE_SHT7X_setUpdateInterval
(
    FTE_OBJECT_PTR  pObj, 
...
static 
FTE_RET  FTE_SHT7X_getChildCount
(
    FTE_OBJECT_PTR  pObj, 
...
static const 
FTE_IFCE_CONFIG FTE_SHT7X_HUMIDITY_defaultConfig =
{
    .xCommon    =
    {
    ...
    .nRegID     = FTE_SHT7X_FIELD_HUMI,
    .nInterval  = FTE_SHT7X_DEFAULT_UPDATE_INTERVAL
};

static const 
FTE_IFCE_CONFIG FTE_SHT7X_TEMPERATURE_defaultConfig =
{
    .xCommon    =
    {
    ...
    .nRegID     = FTE_SHT7X_FIELD_TEMP,
    .nInterval  = FTE_SHT7X_DEFAULT_UPDATE_INTERVAL
};

static const 
FTE_OBJECT_CONFIG_PTR FTE_SHT7X_defaultChildConfigs [] =
{
    (FTE_OBJECT_CONFIG_PTR)&FTE_SHT7X_HUMIDITY_defaultConfig ,
    (FTE_OBJECT_CONFIG_PTR)&FTE_SHT7X_TEMPERATURE_defaultConfig 
};

FTE_SHT7X_CONFIG FTE_SHT7X_defaultConfig =
{
    .xCommon    =
    {
        ...
        .ulChild    = sizeof(FTE_SHT7X_defaultChildConfigs) / sizeof(FTE_OBJECT_CONFIG_PTR),
        .pChild     = (FTE_OBJECT_CONFIG_PTR _PTR_)FTE_SHT7X_defaultChildConfigs
    },
    .nSDA       = FTE_DEV_LWGPIO_SHT_SDA,
    .nSCL       = FTE_DEV_LWGPIO_SHT_SCL,
    .nInterval  = FTE_SHT7X_DEFAULT_UPDATE_INTERVAL,
    .ulDelay    = FTE_SHT7X_DEFAULT_SCL_INTERVAL
};


FTE_VALUE_TYPE  FTE_SHT7X_valueTypes[] =
{
...
{
    .fInit  = FTE_SHT7X_init,
    .fRun   = FTE_SHT7X_run,
    .fStop  = FTE_SHT7X_stop, 
    .fSet   = NULL,
    .fGetInterval   = FTE_SHT7X_getUpdateInterval,
    .fSetInterval   = FTE_SHT7X_setUpdateInterval,
    .fGetStatistics = FTE_SHT7X_statistic,
    .fGetChildCount = FTE_SHT7X_getChildCount,
    .fGetChild      = FTE_SHT7X_getChild,
    .fAttachChild   = FTE_SHT7X_attachChild,
    .fDetachChild   = FTE_SHT7X_detachChild,
};

static 
FTE_LIST _xObjList = {0, NULL, NULL};

FTE_RET   FTE_SHT7X_attach
(
    FTE_OBJECT_PTR  pObj, 
    FTE_VOID_PTR    pOpts
)
{
    ...
    FTE_SHT7X_CONFIG_PTR  pConfig = (FTE_SHT7X_CONFIG_PTR)pObj->pConfig;
    FTE_SHT7X_STATUS_PTR  pStatus = (FTE_SHT7X_STATUS_PTR)pObj->pStatus;
    ...
    pStatus->xCommon.pValue = FTE_VALUE_createArray(FTE_SHT7X_valueTypes, 2);
    ...
    if (FTE_SHT7X_init(pObj) != FTE_RET_OK)
    {
        goto error;
    }
...
)
{
    ...
    FTE_SHT7X_connectionReset(pObj);
...
)
{
    ...
    if (pConfig->nInterval < FTE_SHT7X_DEFAULT_UPDATE_INTERVAL)
    {
        _time_add_msec_to_ticks(&xDTicks, FTE_SHT7X_DEFAULT_UPDATE_INTERVAL);
    }
    else
    {
    ...
    pStatus->hRepeatTimer = _timer_start_periodic_at_ticks(FTE_SHT7X_restartConvert, pObj, TIMER_ELAPSED_TIME_MODE, &xTicks, &xDTicks);
    
    pStatus->bHumidity = TRUE;
    FTE_SHT7X_startConvert(pObj);
    
    _time_init_ticks(&xDTicks, _time_get_ticks_per_sec());
    pStatus->hConvertTimer = _timer_start_oneshot_after_ticks(FTE_SHT7X_done, pObj, TIMER_ELAPSED_TIME_MODE, &xDTicks);
...
)
{
    ...
    if (FTE_FLAG_IS_SET(pStatus->xCommon.xFlags, FTE_SHT7X_FLAG_CONVERT_HUMIDITY))
    {
        FTE_UINT32             nHumidity;
        
        nChildIndex = FTE_SHT7X_FIELD_HUMI;
        if (FTE_SHT7X_getHumidity(pObj, &nHumidity) == FTE_RET_OK)
        {
            FTE_VALUE_setHumidity(&pStatus->xCommon.pValue[FTE_SHT7X_FIELD_HUMI], nHumidity);
            FT_OBJ_STAT_incSucceed(&pStatus->xCommon.xStatistics);
        }
        else
        {
            FTE_VALUE_setValid(&pStatus->xCommon.pValue[FTE_SHT7X_FIELD_HUMI], FALSE);
            FT_OBJ_STAT_incFailed(&pStatus->xCommon.xStatistics);
        }
        
        pStatus->bHumidity = FALSE;
        pStatus->xCommon.xFlags = FTE_FLAG_CLR(pStatus->xCommon.xFlags, FTE_SHT7X_FLAG_CONVERT_HUMIDITY);
    }
    else
    {
        FTE_INT32             nTemperature;
        
        nChildIndex = FTE_SHT7X_FIELD_TEMP;
        
        if (FTE_SHT7X_getTemperature(pObj, &nTemperature) == FTE_RET_OK)
        {
            FTE_VALUE_setTemperature(&pStatus->xCommon.pValue[FTE_SHT7X_FIELD_TEMP], nTemperature);
            FT_OBJ_STAT_incSucceed(&pStatus->xCommon.xStatistics);
        }
        else
        {
            FTE_VALUE_setValid(&pStatus->xCommon.pValue[FTE_SHT7X_FIELD_TEMP], FALSE);
            FT_OBJ_STAT_incFailed(&pStatus->xCommon.xStatistics);
        }

        pStatus->bHumidity = TRUE;
        pStatus->xCommon.xFlags = FTE_FLAG_CLR(pStatus->xCommon.xFlags, FTE_SHT7X_FLAG_CONVERT_TEMPERATURE);
    }
...
)
{
    ...
    if (pBaseConfig == NULL)
    {
        pBaseConfig = (FTE_OBJECT_CONFIG_PTR)&FTE_SHT7X_defaultConfig;
    }
...
void    FTE_SHT7X_SCK_setHi
(
    FTE_OBJECT_PTR  pObj
)
{
    FTE_LWGPIO_setValue(((FTE_SHT7X_STATUS_PTR)pObj->pStatus)->pLWGPIO_SCL, TRUE);
    FTE_SHT7X_delay(1);
}

void    FTE_SHT7X_SCK_setLo
(
    FTE_OBJECT_PTR  pObj
)
{
    FTE_LWGPIO_setValue(((FTE_SHT7X_STATUS_PTR)pObj->pStatus)->pLWGPIO_SCL, FALSE);
    FTE_SHT7X_delay(1);
}

void    FTE_SHT7X_DATA_inMode
(
    FTE_OBJECT_PTR  pObj
)
{
    FTE_LWGPIO_setDirection(((FTE_SHT7X_STATUS_PTR)pObj->pStatus)->pLWGPIO_SDA, LWGPIO_DIR_INPUT);
    FTE_SHT7X_delay(1);
}

void    FTE_SHT7X_DATA_outMode
(
    FTE_OBJECT_PTR  pObj
)
{
    //FTE_LWGPIO_setDirection(((FTE_SHT7X_STATUS_PTR)pObj->pStatus)->pLWGPIO_SDA, LWGPIO_DIR_OUTPUT);
    FTE_SHT7X_delay(1);
}

void    FTE_SHT7X_DATA_setHi
(
    FTE_OBJECT_PTR  pObj
)
{
...
)
{
    ...
    FTE_LWGPIO_getValue(((FTE_SHT7X_STATUS_PTR)pObj->pStatus)->pLWGPIO_SDA, &bValue);

    return  bValue;
}

FTE_RET   FTE_SHT7X_connectionReset(FTE_OBJECT_PTR pObj)
{
    for(FTE_INT32 i = 0 ; i < 9 ; i++)
    {
        FTE_SHT7X_SCK_setHi(pObj);
        FTE_SHT7X_SCK_setLo(pObj);
    }
    FTE_SHT7X_SCK_setHi(pObj);
    FTE_SHT7X_DATA_setLo(pObj);
    FTE_SHT7X_SCK_setLo(pObj);
    FTE_SHT7X_SCK_setHi(pObj);
    FTE_SHT7X_DATA_setHi(pObj);
    FTE_SHT7X_SCK_setLo(pObj);
...
FTE_RET   FTE_SHT7X_startTransmission(FTE_OBJECT_PTR pObj)
{    
    FTE_SHT7X_SCK_setHi(pObj);
    FTE_SHT7X_DATA_setLo(pObj);
    FTE_SHT7X_SCK_setLo(pObj);
...
)
{
    ...
    FTE_SHT7X_DATA_inMode(pObj);
    FTE_SHT7X_SCK_setHi(pObj);
    FTE_BOOL bACK = FTE_SHT7X_DATA_get(pObj);
    FTE_SHT7X_SCK_setLo(pObj);
...
)
{
    ...
    for(FTE_INT32 i = 0 ; i < nData ; i++)
    {
        ...
        for(FTE_INT32 j = 0 ; j < 8 ; j++)
        {
            ...
            if (i < 2)
            {
                if (FTE_SHT7X_DATA_get(pObj) == TRUE)
                {
                    nByte |= 1 << (7 - j);
                }
            }
...
)
{
    ...
    FTE_SHT7X_startTransmission(pObj);
...
)
{
    ...
    {
        pStatus->xCommon.xFlags = FTE_FLAG_SET(pStatus->xCommon.xFlags, FTE_SHT7X_FLAG_CONVERT_HUMIDITY);
        return  _sht_send_cmd(pObj, 0, 5);
    }
    else
    {
        pStatus->xCommon.xFlags = FTE_FLAG_SET(pStatus->xCommon.xFlags, FTE_SHT7X_FLAG_CONVERT_TEMPERATURE);
        return  _sht_send_cmd(pObj, 0, 3);
    }
}

FTE_UINT8 FTE_SHT7X_crc8
(
    FTE_UINT8       ubCRC, 
    FTE_UINT8_PTR   pBuff, 
    FTE_UINT32      nLen
...
FTE_RET   FTE_SHT7X_getTemperature
(
    FTE_OBJECT_PTR  pObj, 
    FTE_INT32_PTR   pTemperature
)
{
    ...
    if ((_sht_read(pObj, &pBuff[1], 3) == FTE_RET_OK) && (FTE_SHT7X_crc8(0x00, pBuff, 4) == 0))
    {
...
)
{
    ...
    if ((_sht_read(pObj, &pBuff[1], 3) == FTE_RET_OK) && (FTE_SHT7X_crc8(ubCRC, pBuff, 4) == 0))
    {
...
FTE_UINT32      FTE_SHT7X_getUpdateInterval
(   
    FTE_OBJECT_PTR  pObj,
    FTE_UINT32_PTR  pulInterval
)
        ...
        case    1:
            {
                ...
                for(i = 0 ; i < ulCount ; i++)
                {
                    ...
                    if (pObj != NULL)
                    {
                        ...
                        FTE_VALUE_toString(&pStatus->xCommon.pValue[FTE_SHT7X_FIELD_TEMP], pBuff, sizeof(pBuff) - 1);
                        printf(" %8s", pBuff);
                        
                        FTE_VALUE_toString(&pStatus->xCommon.pValue[FTE_SHT7X_FIELD_HUMI], pBuff, sizeof(pBuff) - 1);
                        printf(" %8s", pBuff);
        ...
        case    4:
            {
                ...
                if (strcmp(pArgv[2], "scl") == 0)
                {
                    ...
                    if (ulDelay > FTE_SHT7X_DELAY_MAX)
                    {
                        bPrintUsage = TRUE;
                        goto error;
                    }
                    
                    ((FTE_SHT7X_CONFIG_PTR)pObj->pConfig)->ulDelay = ulDelay;
