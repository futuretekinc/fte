 ...
 */
static FTE_TIMER_EVENT MacStateCheckTimer;
 ...
 */
static FTE_TIMER_EVENT ChannelCheckTimer;

/*!
 * LoRaMac duty cycle delayed Tx timer
 */
static FTE_TIMER_EVENT TxDelayedTimer;

/*!
 * LoRaMac reception windows timers
 */
static FTE_TIMER_EVENT RxWindowTimer1;
static FTE_TIMER_EVENT RxWindowTimer2;
 ...
 */
static FTE_TIMER_EVENT AckTimeoutTimer;
...
static uint8_t LoRaMacSetNextChannel( void )
{
    ...
    uint64_t curTime = FTE_TIMER_getCurrentTime( );
    ...
    if( nbEnabledChannels > 0 )
    {
        for( i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < LORA_MAX_NB_CHANNELS; i++ )
        {
            ...
            if( Radio.IsChannelFree( MODEM_LORA, Channels[channelNext].Frequency, RSSI_FREE_TH ) == true )
            {
                ...
                FTE_TIMER_stop( &ChannelCheckTimer );
                return 0;
            }
        }
    }
    ...
    {
        FTE_TIMER_setValue( &ChannelCheckTimer, minTime );
        FTE_TIMER_start( &ChannelCheckTimer );
        LoRaMacState |= MAC_CHANNEL_CHECK;
    }
    return 1;
}
...
void LoRaMacInit( LoRaMacEvent_t *events )
{
    ...
    FTE_TIMER_init( &MacStateCheckTimer, OnMacStateCheckTimerEvent );
    FTE_TIMER_setValue( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );

    FTE_TIMER_init( &ChannelCheckTimer, OnChannelCheckTimerEvent );
    FTE_TIMER_init( &TxDelayedTimer, OnTxDelayedTimerEvent );
    FTE_TIMER_init( &RxWindowTimer1, OnRxWindow1TimerEvent );
    FTE_TIMER_init( &RxWindowTimer2, OnRxWindow2TimerEvent );
    FTE_TIMER_init( &AckTimeoutTimer, OnAckTimeoutTimerEvent );
...
uint8_t LoRaMacSendFrameOnChannel( ChannelParams_t channel )
{
    ...
    FTE_TIMER_start( &MacStateCheckTimer );
  
    if( MAX( Bands[channel.Band].TimeOff, AggregatedTimeOff ) > ( FTE_TIMER_getCurrentTime( ) ) )
    {
        // Schedule transmission
        FTE_TIMER_setValue( &TxDelayedTimer, MAX( Bands[channel.Band].TimeOff, AggregatedTimeOff ) );
        FTE_TIMER_start( &TxDelayedTimer );
    }
    else
    {
        // Send now
...
static void OnRadioTxDone( void )
{
    ...
    {
        FTE_TIMER_setValue( &RxWindowTimer1, RxWindow1Delay );
        FTE_TIMER_start( &RxWindowTimer1 );
        if( LoRaMacDeviceClass != CLASS_C )
        {
            FTE_TIMER_setValue( &RxWindowTimer2, RxWindow2Delay );
            FTE_TIMER_start( &RxWindowTimer2 );
        }
    }
    else
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                    ...
                    ( micRx == mic ) )
                {
                    ...
                    if( fCtrl.Bits.Ack == 1 )
                    {
                        ...
                        // are needed.
                        FTE_TIMER_stop( &AckTimeoutTimer );
                    }
                    else
                    {
                        LoRaMacEventInfo.TxAckReceived = false;
...
static void OnRxWindow2TimerEvent( void )
{
    ...
    {
        FTE_TIMER_setValue( &AckTimeoutTimer, ACK_TIMEOUT + randr( -ACK_TIMEOUT_RND, ACK_TIMEOUT_RND ) );
        FTE_TIMER_start( &AckTimeoutTimer );
    }
