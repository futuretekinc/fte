 ...
 * Tx and Rx timers
 */
FTE_TIMER_EVENT TxTimeoutTimer;
FTE_TIMER_EVENT RxTimeoutTimer;
FTE_TIMER_EVENT RxTimeoutSyncWord;
...
void SX1276Init( RadioEvents_t *events )
{
    ...
    FTE_TIMER_init( &TxTimeoutTimer, SX1276OnTimeoutIrq );
    FTE_TIMER_init( &RxTimeoutTimer, SX1276OnTimeoutIrq );
    FTE_TIMER_init( &RxTimeoutSyncWord, SX1276OnTimeoutIrq );
...
{
    FTE_TIMER_stop( &RxTimeoutTimer );
    FTE_TIMER_stop( &TxTimeoutTimer );
...
void SX1276SetRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        FTE_TIMER_setValue( &RxTimeoutTimer, timeout );
        FTE_TIMER_start( &RxTimeoutTimer );
    }
    ...
    if( SX1276.Settings.Modem == MODEM_FSK )
    {
        ...
        {
            FTE_TIMER_setValue( &RxTimeoutSyncWord, (uint32_t)(( 8.0 * ( SX1276.Settings.Fsk.PreambleLen +
                                                         ( ( SX1276Read( REG_SYNCCONFIG ) &
                                                            ~RF_SYNCCONFIG_SYNCSIZE_MASK ) +
                                                         1.0 ) + 10.0 ) /
                                                        ( double )SX1276.Settings.Fsk.Datarate ) * 1e6) );
            FTE_TIMER_start( &RxTimeoutSyncWord );
        }
    }
    else
    {
...
void SX1276SetTx( uint32_t timeout )
{
    FTE_TIMER_setValue( &TxTimeoutTimer, timeout );
    ...
    FTE_TIMER_start( &TxTimeoutTimer );
...
void SX1276OnTimeoutIrq( void )
{
    ...
    case RF_RX_RUNNING:
        if( SX1276.Settings.Modem == MODEM_FSK )
        {
            ...
            {
                SX1276.Settings.State = RF_IDLE;
                FTE_TIMER_stop( &RxTimeoutSyncWord );
            }
        }
...
uint_8 SX1276LoRaGetOpMode( void )
{
    ...
    return ubObMode & ~RFLR_OPMODE_MASK;
}
