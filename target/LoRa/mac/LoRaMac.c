...
uint8_t LoRaMacSendFrameOnChannel( ChannelParams_t channel )
{
    ...
    { // High Speed FSK channel
        Radio.SetTxConfig( MODEM_FSK, TxPowers[ChannelsTxPower], 25e3, 0, (uint32_t)(Datarates[ChannelsDatarate] * 1e3), 0, 5, false, true, 0, 0, false, 3e6 );
            ...
            case SRV_MAC_RX_TIMING_SETUP_REQ:
                {
                    ...
                    ReceiveDelay1 = (uint32_t)(delay * 1e6);
                    ReceiveDelay2 = (uint32_t)(ReceiveDelay1 + 1e6);
...
void LoRaMacRxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    if( Radio.Status( ) == RF_IDLE )
    {
        ...
        if( datarate == DR_7 )
        {
            Radio.SetRxConfig( MODEM_FSK, 50e3, (uint32_t)(Datarates[datarate] * 1e3), 0, 83.333e3, 5, 0, false, 0, true, 0, 0, false, rxContinuous );
        }
        else
        {
